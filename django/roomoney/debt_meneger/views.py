...
def add_debt(request):
	...
	if request.method == "POST":
		...
		return HttpResponseRedirect(f'http://127.0.0.1:8000/debt/display_transaction/{lowaner[0].name}')
...
def create_new_partner(request, name, date_of_berith, phone, email):
	...
	new_partner.save()
	return HttpResponseRedirect(f'http://127.0.0.1:8000/debt/display_partner/{name}')
...
def display_partner_debt_sum(request):
	...
	if request.method == 'POST':
		...
		partner_lowns = [i.mouny_moved for i in partner_lowns]
		partner_debts= Transaction.objects.filter(debtor__name=partner_name)
		partner_debts = [i.mouny_moved for i in partner_debts]
		partner_total = subtract_lowns_from_debt(partner_lowns, partner_debts)
		context = {	'partner_debts' : partner_debts, 
